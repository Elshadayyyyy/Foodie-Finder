...
import { getMealById, Meal } from '@/services/mealApi';
...
const MealDetailPage: React.FC = () => {
  ...
  if (error) {
    return (
      <div className="text-center text-destructive text-xl mt-8">
        <p>{error}</p>
        <Link to="/">
          <Button className="mt-4 bg-primary hover:bg-primary-600 text-primary-foreground">Go Home</Button>
        </Link>
      </div>
    );
  }
  ...
  const youtubeVideoId = meal.strYoutube ? meal.strYoutube.split('v=')[1] : null;

  return (
    <div className="max-w-4xl mx-auto p-4 md:p-8">
      <Button asChild variant="outline" className="mb-6 border-border text-foreground hover:bg-primary hover:text-primary-foreground">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <div className="md:col-span-1 flex flex-col justify-center">
            ...
            <div className="text-lg text-muted-foreground mb-4 space-y-1">
              <p><strong>Category:</strong> <span className="font-semibold text-foreground">{meal.strCategory}</span></p>
              <p><strong>Area:</strong> <span className="font-semibold text-foreground">{meal.strArea}</span></p>
            </div>
          </div>
        </div>
        <CardContent className="p-0 mt-8"> 
          <h2 className="text-2xl font-semibold text-foreground mb-3 border-b border-border pb-2">Ingredients:</h2>
          ...
          <h2 className="text-2xl font-semibold text-foreground mb-3 border-b border-border pb-2">Instructions:</h2>
          ...
          {youtubeVideoId && (
            <>
              <h2 className="text-2xl font-semibold text-foreground mb-3 border-b border-border pb-2">Watch on YouTube:</h2>
